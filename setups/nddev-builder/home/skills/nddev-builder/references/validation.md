...
```bash
target="$(mktemp -d)/grok-target"
mkdir -p "$target"
grok-setup-system install baseline    --target "$target"
grok-setup-system status              --target "$target"
grok-setup-system select full-auto    --target "$target"
grok-setup-system diff                --target "$target"
grok-setup-system backups             --target "$target"
grok-setup-system restore             --target "$target"
grok-setup-system remove              --target "$target"
```
...
Ask `grok-setup-system provider-info` for `harness_id`; that is the value
`--harness` takes, and it is not always the directory name.

```bash
ai-stp provider conformance --harness <harness_id> \
  --executable target/release/grok-setup-system \
