...
The Grok Build provider retains the existing protocol and preservation contract.
...
The Grok Build provider retains prior setup identity and written ownership.
...
declares.** The rule a model reads while working on Grok Build said the product
...
**A walk written when a surface held one kind.** Grok Build's table gained a
...
**Grok Build's table gained a kind.** Where a product loads a plugin from a
...
**Grok Build declares the `instruction` kind**, and the surface that routes it is
...
Grok Build's provider publishes a projection profile per *scope* -- a second
...
Starts Grok Build, and takes over a target the estate before this one
still claims.
...
Installs Grok Build itself, not only its configuration.
...
Grok Build harness configuration in a caller-named target directory.
