...
Installs, reselects, restores and removes a complete Grok Build harness configuration, and owns the program lifecycle.
...
grok-setup-system select minimal      --target ~/.tool-config
...
grok-setup-system restore --backup slot-000000000002 --target ~/.tool-config
...
Point `GROK_SETUP_SYSTEM_SETUP_CATALOG` at a directory to use setups of your own.

## What it manages

| | |
| --- | --- |
| Product | Grok Build (xAI) |
| Documented configuration home | `~/.grok` |
...
```bash
GROK_INSTALL_DIR=/opt/grok-setup-system sh install.sh
```

The same variable on both scripts, and it is `GROK_INSTALL_DIR`
...
```bash
ai-stp provider conformance --harness grok-build \
  --executable ~/.local/bin/grok-setup-system \
...
docker run --rm -v "$HOME/.config:/config" \
  ghcr.io/nddev-opennetwork/grok-setup-system:0.0.7 \
